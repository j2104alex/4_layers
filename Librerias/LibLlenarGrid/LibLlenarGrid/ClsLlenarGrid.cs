...
namespace LibLlenarGrid
{
    public class ClsLlenarGrid
    {
        ...
        //dataGridView es una cuadricula
        public bool LlenarGrid(DataGridView grdGenerico)
        {
            if (strSQL == "") {
                strError = "debe definir una instruccion SQL";
            }
            //instancia que permite traer el metodo llenarDataSet que es un conjunto de tablas que
            //recide en la memoria RAM
            ClsConexion objConexionBD = new ClsConexion();
            if (strNombreTabla == "") {
                strNombreTabla = "LlenarGrid";
            }
            //recibe dos parametros, nombre de tabla y la sentencia del procedimiento almacenado
            if (objConexionBD.LlenarDataSet(strNombreTabla, strSQL, false))
            {
                //es el que obtiene la informacion del procedimiento almacenado
